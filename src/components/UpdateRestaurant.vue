...
export default {
    ...
    setup(props) {
        ...
        async function handleForm(){
            ...
            let result = await axios.put('http://localhost:3001/restaurants/'+props.id, restaurant)
            restaurant.name = ''
            restaurant.address = ''
            restaurant.contact = ''
        ...
        onMounted(async () => {
            ...
            let item = await axios.get('http://localhost:3001/restaurants/'+props.id)
            restaurant.name = item.data.name
            restaurant.contact = item.data.contact
            restaurant.address = item.data.address
        });
        return{
            handleForm,
            ...toRefs(restaurant)
